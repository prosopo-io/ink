...
pub struct ItemImpl {
    ...
    /// # Note
    ///
    /// User can provide label of trait which will be used inside of metadata.
...
impl ItemImpl {
    ...
    /// Returns the trait label of the implementation block if any has been provided.
    ///
    /// # Note
    ///
    /// If label is `""`, it means that user wants to remove trait labeling in metadata
    pub fn trait_label(&self) -> Option<String> {
        if self.trait_label.is_some() {
            let name = self.trait_label.clone();
    ...
    /// Returns the trait label provided by user.
    pub fn user_provided_trait_label(&self) -> Option<&String> {
        self.trait_label.as_ref()
    }
