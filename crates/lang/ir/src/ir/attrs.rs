...
impl InkAttribute {
    ...
    pub fn label(&self) -> Option<String> {
        self.args().find_map(|arg| {
            if let ir::AttributeArg::Label(name) = arg.kind() {
...
pub enum AttributeArg {
    ...
    /// with traits to manually control their labeling inside of metadata.
    Label(String),
    /// `#[ink(namespace = "my_namespace")]`
    ///
...
impl AttributeArg {
    ...
    pub fn kind(&self) -> AttributeArgKind {
        match self {
            ...
            Self::Label(_) => AttributeArgKind::Label,
                    ...
                    syn::Meta::NameValue(name_value) => {
                        ...
                        if name_value.path.is_ident("label") {
                            if let syn::Lit::Str(lit_str) = &name_value.lit {
                                ...
                                return Ok(AttributeFrag {
                                    ast: meta,
                                    arg: AttributeArg::Label(
                                        name,
                                    ),
                                })
                            }
                            return Err(format_err!(name_value, "expecteded string type for `label` argument, e.g. #[ink(label = \"transfer\")]"))
                        }
...
mod tests {
    ...
    fn label_works() {
        assert_attribute_try_from(
            ...
            Ok(test::Attribute::Ink(vec![AttributeArg::Label(
